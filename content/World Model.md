...
也有种特别直观的分类方式：语言模型能预测下一个 token，可以说它是文本世界模型。视频模型预测下一段视频，可以说它是视觉世界模型。游戏模型根据玩家输入生成下一帧，也可以叫交互式世界模型。
...
一句话概括当然是尽量好用。但就像其他领域一样，为了便于从细微之处完善模型，我们需要在各个方面描述模型的各种性能。具体期待什么去看benchmark就行。
