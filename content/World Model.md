...
只要模型能够根据当前世界和动作，持续生成合理的未来画面，它就相当于在内部模拟世界，因为视频中已经包含大量关于世界运行规律的信息。

#### 输入输出

一般是当前画面 + 历史 + 动作 → 动作发生后的未来视频。以下是常见的输入输出形式。
...
将原始视频的像素和输入的动作压缩成token or latent，有时也输入text prompt，使用自回归或扩散得到预测的state，将这个predicted state解码成视频。不断重复这个过程。

#### 分支

- 通用视频模拟派：利用大规模互联网视频学习广泛的世界规律。
- 可交互游戏世界派：生成一个人或智能体可以实时操作的世界。
- 机器人视频模拟派：模拟语言指令和实际动作的视觉结果，优化策略。
- 自动驾驶视频模拟派：和机器人类似，只是针对汽车。
- 世界基础模型派：训练一个大型通用视频模型，再针对不同任务进行微调。
...
比如重建：根据图片还原相机位置，判断物体位置、形状、外观，渲染新视角画面

#### 分支

- 三维重建派：从二维观察恢复三维空间。
- 三维生成派：从文字、图片或粗略布局生成一个新的三维世界。
- 4D 动态场景派：建立一个会随时间变化的三维场景。
- 可交互空间世界派：根据已有3d和动作生成未来的3d。
